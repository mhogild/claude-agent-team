...
would have to become true for this to be worth building. If you're inventing those answers, stop
and ask the human. A confident fabrication here is the most expensive kind of error the team makes,
...
2. The four risks are each either **retired with evidence** or **explicitly accepted** by the human,
   in writing, with the reason.
3. The spec is approved by the human, with testable acceptance criteria.
4. Out-of-scope is stated, and dependencies / integration risks are named.

"We'll figure it out in build" is an escalation, not a plan.
